...
EMSCRIPTEN_BINDINGS(binfhe_serial_em) {
  ...
  emscripten::function("SerializeRefreshKeyToBuffer",
                       &SerializeToBuffer<std::shared_ptr<RingGSWACCKey>>);

  emscripten::function("SerializeSwitchingKeyToBuffer",
                       &SerializeToBuffer<std::shared_ptr<LWESwitchingKey>>);
  ...
  // use shared ptr because some of thse types don't have copy / move constructors
  emscripten::function("DeserializeRefreshKeyFromBuffer",
                       &DeserializeFromBuffer<std::shared_ptr<RingGSWACCKey>>);
